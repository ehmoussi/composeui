...
@dataclass(eq=False)
class ItemsView(View, IItemsView):
    r"""Items view."""

    table_widget: ItemsWidget = field(init=False)
    filter_view: FilterTableView = field(init=False)
    pagination_view: PaginationView = field(init=False)
...
@dataclass(eq=False)
class ItemsGroupView(ItemsView, GroupView):
