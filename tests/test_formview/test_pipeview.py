...
from examples.formview.pipeform import EdgeType, IPipeFormView
...
@pytest.fixture()
def app_pipe(app: FormViewApp) -> Tuple[IPipeFormView, IExampleMainView, Model]:
    return app.main_view.pipe_view, app.main_view, app.model


def test_update_name(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
...
def test_update_export_path(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
...
def test_update_main_radius(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
    view, _, model = app_pipe
...
def test_update_edge_type(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
    view, _, model = app_pipe
...
def test_color(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
    view, _, _ = app_pipe
    # initial state
...
def test_is_visible(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
    view, _, _ = app_pipe
    # initial state
...
def test_is_enabled(app_pipe: Tuple[IPipeFormView, IExampleMainView, Model]) -> None:
    view, _, _ = app_pipe
